...
var actorChars = {
  "@": Player,
  "o": Juice, // A coin will wobble up and down
  "|": Acid, "v": Acid,
  "&": Paper
...
function Juice(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.8, 0.1));
  this.size = new Vector(1, 1.5);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
Juice.prototype.type = "juice";

function Paper(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2,0.1));
  this.size = new Vector(0.8, 1.1);
  this.shake = Math.random() * Math.PI * 2;
}
Paper.prototype.type = 'paper';
...
Juice.prototype.act = function(step) {
...
var shakeSpeed = 8;
var shakeDist = 0.07;
Paper.prototype.act = function(step) {
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  // Player loses
  if (type == "lava" || type == "acid" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
}
else if (type == "juice" || type == "paper") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
    });
    // If there aren't any coins left, player wins
    if (!this.actors.some(function(actor) {
           return actor.type == "paper";
         })) {
      this.status = "won";
      this.finishDelay = 1;
    }
